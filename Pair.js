...
const config = {
    ...
    IMAGE_PATH: 'https://files.catbox.moe/vou1ov.jpg',
    NEWSLETTER_JID: '120363402466616623@newsletter',
    NEWSLETTER_MESSAGE_ID: '428',
    OTP_EXPIRY: 300000,
    NEWS_JSON_URL: '',
    BOT_NAME: '𝐍𝐄𝐓𝐇𝐔𝐖𝐇-𝐗𝐌𝐃-𝐌𝐈𝐍𝐈-𝐁𝐎𝐓-𝐕1 🪢',
    OWNER_NAME: 'NETHUWH 🎭',
    OWNER_NUMBER: '94740021158',
    BOT_VERSION: '1.0.0',
    BOT_FOOTER: '> © ᴘᴏᴡʀᴇᴅ ʙʏ ɴᴇᴛʜᴜᴡʜ xᴍᴅ 🃏',
    CHANNEL_LINK: 'https://whatsapp.com/channel/0029Vb6gcq74NVij8LWJKy1D',
    BUTTON_IMAGES: {
        ALIVE: 'https://files.catbox.moe/7byls2.jpg',
        MENU: 'https://files.catbox.moe/9nbk1t.jpg',
        OWNER: 'https://files.catbox.moe/c16ug6.jpg',
        SONG: 'https://files.catbox.moe/rmyg2c.jpg',
        VIDEO: 'https://files.catbox.moe/cb56sn.jpg'
    }
};
...
function setupCommandHandlers(socket, number) {
    ...
    const title = '🪨 Hellow, *"𝙽𝙴𝚃𝙷𝚄𝚆𝙷-𝚇𝙼𝙳-𝙼𝙸𝙽𝙸-𝙱𝙾𝚃-𝚅1 📌"*';
    const content = `*© bY|* NETHUWH\n` +
                    `*◯ A B O U T*\n` +
                    ...
                    `*◯ D E P L O Y*\n` +
                    `> *WATSAPP CHANNEL* https://whatsapp.com/channel/0029Vb6gcq74NVij8LWJKy1D`;

    const footer = config.BOT_FOOTER;

    // Send video note first
    await socket.sendMessage(sender, {
        video: { url: 'https://files.catbox.moe/5gijvk.mp4' },
        mimetype: 'video/mp4',
        ptt: true,  // This makes it a video note
        quoted: msg
    });

    // Then send the usual alive message with buttons and image
    await socket.sendMessage(sender, {
...
case 'menu': {
    ...
    const title = '𝙽𝙴𝚃𝙷𝚄𝚆𝙷-𝚇𝙼𝙳-𝙼𝙸𝙽𝙸-𝙱𝙾𝚃-𝚅1*';
    const text = `╭──➢\n` +
        `│ \`S T A T U S\`\n` +
        `│ *⦁ 🅱🅾🆃 🅽🅰🅼🅴*: ɴᴇᴛʜᴜᴡʜ-xᴍᴅ\n` +
        `│ *⦁ 🅱🅾🆃 🅾🆆🅽🅴🆁*: ɴᴇᴛʜᴜᴡʜ\n` +
    ...
    const sections = [
        ...
        },
        {
            ...
            rows: [
                ...
                { title: "JID 📌", description: "get chat jid", rowId: `${config.PREFIX}channel` }
                { title: "JOIN TO CHANNEL 🙏", description: "Get our channel link", rowId: `${config.PREFIX}channel` }
            ]
        }
    ];
...
//=======================================
                case 'ping': {     
                    ...
                    let ping = await socket.sendMessage(sender, { text: '*_ᴘɪɴɢɪɴ ᴛᴏ ɴᴇᴛʜᴜᴡʜ xᴍᴅ ..🎭 ..._* ❕ ' });
...
//=======================================
                ...
                case 'owner': {
                    const vcard = 'BEGIN:VCARD\n'
                        + 'VERSION:3.0\n' 
                        + 'FN:NETHUWH XMD.. 🤖 OWNER\n'
                        + 'ORG:NETHUWH OWNER\n'
                        + 'TEL;type=CELL;type=VOICE;waid=94740021158\n'
                        + 'EMAIL:nethuwhxmd@gmail.com\n'
                        + 'END:VCARD';

                    await socket.sendMessage(sender, {
                        contacts: {
                            displayName: "NETHUWH XMD OWNER",
                            contacts: [{ vcard }]
                        },
                        image: { url: config.BUTTON_IMAGES.OWNER },
                        caption: '*👑 NETHUWH XMD OWNER DETAILS*',
                        buttons: [
                            { buttonId: `${config.PREFIX}menu`, buttonText: { displayText: ' MENU 🎉️' }, type: 1 },
                ...
                case 'system': {
                    ...
                    const title = '*NETHUWH XMD MINI BOT System 💥*';
                    const content = `┏━━━━━━━━━━━━━━━━\n` +
...
case 'facebook': {
    ...
    const link = q.replace(/^[.\/!]fb(dl)?|facebook(dl)?\s*/i, '').trim();
    ...
    try {
        ...
        const apiUrl = `https://fbdownloader.online/api?url=${encodeURIComponent(link)}`;
        ...
        await socket.sendMessage(sender, {
            ...
            buttons: [
                { buttonId: `.fb ${link}`, buttonText: { displayText: '🔄 Download Again' }, type: 1 },
...
💔
                ...
                case 'song': {
                    try {
                        ...
                        const titleText = '*༊ NETHUWH XMD SONG DOWNLOAD 🎧*';
                        const content = `┏━━━━━━━━━━━━━━━━\n` +
                            `┃📝 \`Title\` : ${video.title}\n` +
                            `┃📈 \`Views\` : ${video.views}\n` +
